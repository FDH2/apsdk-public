...
ap_airplay_connection::ap_airplay_connection(asio::io_context &io_ctx, ap_config_ptr &config, ap_handler_ptr &handler,
  ...
  timing_sync_service_ = std::make_shared<ap_mirroring_timing_sync_service>();
  timing_sync_service_->open();
  initialize_request_handlers();
  LOGD() << "ap_airplay_connection (" << std::hex << this << ") is being created";
}

ap_airplay_connection::~ap_airplay_connection() {
  printf("*** ap_airplay_connection::~ap_airplay_connection\n");
  ...
  if (is_reversed()) {
    ap_casting_event_connection_manager::get().remove(apple_session_id_);
  }
  ...
  } else if (mirroring_session == session_type_) {
    mirroring_session_handler_.reset();
    if (handler_) {
      handler_->on_session_end(session_id_);
    }
  }

  LOGD() << "ap_airplay_connection (" << std::hex << this << ") is being destroyed";
}
...
uint32_t ap_airplay_connection::get_session_type() {
  printf("*** ap_airplay_connection::get_session_type: %u\n", session_type_);
  return session_type_; }

void ap_airplay_connection::set_mirroring_handler(ap_mirroring_session_handler_ptr handler) {
  printf("*** ap_airplay_connection::set_mirroring_handler\n");
...
void ap_airplay_connection::set_casting_handler(ap_casting_session_handler_ptr handler) {
  printf("*** ap_airplay_connection::set_casting_handler\n");
  if (casting_session == session_type_) {
    video_session_handler_ = handler;
  }
}

void ap_airplay_connection::disconnect() {
  printf("*** ap_airplay_connection::disconnect\n");
  stop(); }

void ap_airplay_connection::options_handler(const request &req, response &res) {
  printf("*** ap_airplay_connection::options_handler\n");
...
void ap_airplay_connection::post_pair_setup_handler(const request &req, response &res) {
  printf("*** ap_airplay_connection::pair_setup_handler\n");
  //DUMP_REQUEST_WITH_CONNECTION(req);
  print_request(req, "");
  if (32 == req.content.size()) {
    crypto_->init_client_public_keys(nullptr, 0, req.content.data(), 32);
  }
  res.with_status(ok).with_content_type(APPLICATION_OCTET_STREAM).with_content(crypto_->server_keys().ed_public_key());
}

void ap_airplay_connection::post_pair_verify_handler(const request &req, response &res) {
  printf("*** ap_airplay_connection::pair_verify_handler\n");
  //DUMP_REQUEST_WITH_CONNECTION(req);
  print_request(req, "");
  auto *header = (pair_verify_header_t *)req.content.data();
  if (header->is_first_frame) {
    crypto_->init_client_public_keys(req.content.data() + 4,
                                     32, // client curve public key
                                     req.content.data() + 4 + 32,
                                     32); // client ed public key
    ...
    res.with_status(ok).with_content_type(APPLICATION_OCTET_STREAM).with_content(res_data);
    printf("*** response: post_pair_verify_handler: first_frame:\n");
    return;
  } else {
    if (crypto_->verify_pair_signature(req.content.data() + 4, 64)) {
      res.with_status(ok);
      printf("*** response: post_pair_verify_handler: second_frame:\n");
    } else {
...
void ap_airplay_connection::post_fp_setup_handler(const request &req, response &res) {
  printf("*** ap_airplay_connection::post_fp_setup_handler\n");
  //DUMP_REQUEST_WITH_CONNECTION(req);
  print_request(req, "");
  auto *header = (fp_header_t *)req.content.data();
  if (header->signature[0] == 'F' && header->signature[1] == 'P' && header->signature[2] == 'L' &&
      header->signature[3] == 'Y') {
	...
	printf("*** response octet stream, fp_setup phase 0x01\n");
      } else if (header->phase == 0x02) {
	printf("*** response octet stream, fp_setup phase 0x02 (unsupported)\n");
        ...
        res.with_status(ok).with_content_type(APPLICATION_OCTET_STREAM).with_content(content);
	printf("*** response octet stream, fp_setup phase 0x03\n");
      } else {
...
void ap_airplay_connection::setup_handler(const request &req, response &res) {
  printf("*** ap_airplay_connection::setup_handler\n");
  //DUMP_REQUEST_WITH_CONNECTION(req);
  print_request(req, "");
  do {
    ...
    if (streams) {
      ...
      if (stream_type_t::audio == type) {
        ...
        auto format_obj = plist_object_dict_get_value(stream_obj, "audioFormat");
        if (!format_obj || PLIST_TYPE_INTEGER != plist_object_get_type(format_obj))
          break;
        ...
        if (!mirroring_audio_stream_service_) {
          mirroring_audio_stream_service_ =
              std::make_shared<ap_mirroring_audio_stream_service>(crypto_, mirroring_session_handler_);
          mirroring_audio_stream_service_->start();
          LOGI() << "mirroring service audio data port: " << mirroring_audio_stream_service_->data_port()
                 << ", control port: " << mirroring_audio_stream_service_->control_port();
        }

        if (mirroring_session_handler_) {
          mirroring_session_handler_->on_audio_stream_started((audio_data_format_t)format);
        }
        ...
        // clang format-on
	printf("*** response plist, setup_handler, stream_type audio:\n");
        res.with_status(ok)
	  ...
	  plist_object_dict_get_value(stream_obj, "streamConnectionID");
        ...
        if (!mirroring_video_stream_service_) {
          mirroring_video_stream_service_ =
              std::make_shared<ap_mirroring_video_stream_service>(crypto_, 0, mirroring_session_handler_);
          mirroring_video_stream_service_->start();
          LOGI() << "mirroring service video port: " << mirroring_video_stream_service_->port();
        }
        ...
        // clang-format on
	printf("*** response plist, setup_handler, stream_type video:\n");
        res.with_status(ok).with_content_type(APPLICATION_BINARY_PLIST).with_content(video_stream.to_bytes_array());
      ...
      session_type_ = mirroring_session;
      if (handler_) {
...
void ap_airplay_connection::get_info_handler(const request &req, response &res) {
  printf("*** ap_airplay_connection::get_info_handler\n");
  //DUMP_REQUEST_WITH_CONNECTION(req);
  print_request(req, "");
  // clang-format off
  ...
  // clang-format on
  printf("*** response plist, get_info_handler:\n");
...
void ap_airplay_connection::post_feedback_handler(const request &req, response &res) {
  printf("*** ap_airplay_connection::post_feedback_handler\n");
  print_request(req, "");
  // DUMP_REQUEST_WITH_CONNECTION(req);
  res.with_status(ok);
}
...
void ap_airplay_connection::record_handler(const request &req, response &res) {
  printf("*** ap_airplay_connection::record_handler\n");
  //DUMP_REQUEST_WITH_CONNECTION(req);
  print_request(req, "");
  printf("*** ap_airplay_connection::record_handler\n");
...
void ap_airplay_connection::get_parameter_handler(const request &req, response &res) {
  printf("*** ap_airplay_connection::get_parameter_handler\n");  
  //DUMP_REQUEST_WITH_CONNECTION(req);
  print_request(req, "");
...
void ap_airplay_connection::post_audioMode(const request &req, response &res) {
  printf("*** ap_airplay_connection::post_audioMode\n");
  //DUMP_REQUEST_WITH_CONNECTION(req);
  print_request(req, "");
  res.with_status(ok);
}

void ap_airplay_connection::set_parameter_handler(const request &req, response &res) {
  printf("*** ap_airplay_connection::set_parameter_handler\n");
  //DUMP_REQUEST_WITH_CONNECTION(req);
  print_request(req, "");
  ...
  if (0 == req.content_type.compare(TEXT_PARAMETERS)) {
    ...
    // volume: -11.123877
    if (ap_casting_content_parser::get_volume_value(volume, content.c_str())) {
      if (mirroring_session_handler_) {
        if (0 == volume)
          ratio = 100;
        ...
        mirroring_session_handler_->on_audio_set_volume(ratio, volume);
      }
    }
    // progress: 1146221540/1146549156/1195701740  start/current/end
    else if (ap_casting_content_parser::get_progress_values(start, current, end, content.c_str())) {
      if (mirroring_session_handler_) {
        ratio = 100.0f * (current - start) / (end - start);
        mirroring_session_handler_->on_audio_set_progress(ratio, start, current, end);
      }
    }
  } else if (0 == req.content_type.compare(IMAGE_JPEG) || 0 == req.content_type.compare(IMAGE_PNG)) {
    // body is image data
    if (mirroring_session_handler_) {
      mirroring_session_handler_->on_audio_set_cover(
          req.content_type, req.content.data(), (uint32_t)req.content.size());
    }
  } else if (0 == req.content_type.compare(APPLICATION_DMAP_TAGGED)) {
    // body is dmap data
    if (mirroring_session_handler_) {
      mirroring_session_handler_->on_audio_set_meta_data(req.content.data(), (uint32_t)req.content.size());
    }
  } else {
...
void ap_airplay_connection::teardown_handler(const request &req, response &res) {
  printf("ap_airplay_connection::teardown_handler\n");
  //DUMP_REQUEST_WITH_CONNECTION(req);
  print_request(req, "");
        ...
        if (stream_obj && PLIST_TYPE_DICT == plist_object_get_type(stream_obj)) {
          ...
          if (type_obj && PLIST_TYPE_INTEGER == plist_object_get_type(type_obj)) {
            ...
            if (stream_type_t::video == type) {
              // Stop video stream
              if (mirroring_video_stream_service_) {
                mirroring_video_stream_service_->stop();
                mirroring_video_stream_service_.reset();
              }
              ...
              // Stop audio stream
              if (mirroring_audio_stream_service_) {
                mirroring_audio_stream_service_->stop();
                mirroring_audio_stream_service_.reset();
              }

              if (mirroring_session_handler_) {
                mirroring_session_handler_->on_audio_stream_stopped();
              }
...
void ap_airplay_connection::flush_handler(const request &req, response &res) {
  printf("*** ap_airplay_connection::flush_handler\n");
  //DUMP_REQUEST_WITH_CONNECTION(req);
  print_request(req, "");
  ...
  if (rtp_info_header != req.headers.end()) {
    ...
    if (!strncmp(rtp_info.c_str(), "seq=", 4)) {
      uint64_t next_seq = strtol(rtp_info.c_str() + 4, nullptr, 10);
      (void)next_seq;
      // Flush
    }
  }
...
void ap_airplay_connection::get_server_info_handler(const request &req, response &res) {
  printf("ap_airplay_connection::get_server_info_handler\n");
  //DUMP_REQUEST_WITH_CONNECTION(req);
  print_request(req, "");
  res.with_status(ok);
  // return;
  ...
  session_type_ = casting_session;
  if (handler_) {
    handler_->on_session_begin(shared_from_this());
  }
  printf("*** response plist, get_server_info_handler:\n");
  res.with_status(ok).with_content_type(TEXT_APPLE_PLIST_XML).with_content(oss.str());
}

void ap_airplay_connection::post_fp_setup2_handler(const request &req, response &res) {
  printf("*** ap_airplay_connection::fp_setup2_handler\n");
  //DUMP_REQUEST_WITH_CONNECTION(req);
  print_request(req, "");
  res.with_status(ok);
}

void ap_airplay_connection::post_reverse_handler(const request &req, response &res) {
  printf("*** ap_airplay_connection::post_reverse_handler\n");
  //DUMP_REQUEST_WITH_CONNECTION(req);
  print_request(req, "");
...
void ap_airplay_connection::post_play_handler(const request &req, response &res) {
  printf("*** ap_airplay_connection::post_play_handler\n");
  //DUMP_REQUEST_WITH_CONNECTION(req);
  print_request(req, "");
  std::string location;

  if (0 == compare_string_no_case(req.content_type.c_str(), APPLICATION_BINARY_PLIST)) {
    ...
    auto content_location_obj = plist_object_dict_get_value(data_obj, "Content-Location");
    ...
    auto start_position_obj = plist_object_dict_get_value(data_obj, "Start-Position-Seconds");
  ...
  } else if (0 == compare_string_no_case(req.content_type.c_str(), TEXT_PARAMETERS) ||
             0 == compare_string_no_case(req.content_type.c_str(), APPLICATION_OCTET_STREAM)) {
    // The request content is text:
    // req.body:
    //      Content-Location:URL
    //      Start-Position: float
    std::string content(req.content.begin(), req.content.end());
    if (!ap_casting_content_parser::get_play_parameters(location, start_pos_in_ms_, content.c_str())) {
      LOGW() << "No start position found, set to 0";
    }
  } else {
    LOGW() << "Unknown content type :" << req.content_type << " for uri: " << req.uri;
    res.with_status(bad_request);
    return;
  }
  ...
  if (!ap_casting_media_data_store::get().request_media_data(location, it_session_id->second)) {
    // Normal URL
    if (video_session_handler_) {
      video_session_handler_->on_video_play(session_id_, location, start_pos_in_ms_);
    }
  }
...
void ap_airplay_connection::post_scrub_handler(const request &req, response &res) {
  printf("*** ap_airplay_connection::scrub_handler\n");
  //DUMP_REQUEST_WITH_CONNECTION(req);
  print_request(req, "");
  // /scrub?position=1298.000000
  float pos_in_ms = 0.0f;
  if (ap_casting_content_parser::get_scrub_position(pos_in_ms, req.uri.c_str())) {
    if (video_session_handler_) {
...
void ap_airplay_connection::post_rate_handler(const request &req, response &res) {
  printf("*** ap_airplay_connection::post_rate_handler\n");
  //DUMP_REQUEST_WITH_CONNECTION(req);
  print_request(req, "");
  // /rate?value=0.000000
  float value = 0.0f;
  if (ap_casting_content_parser::get_rate_value(value, req.uri.c_str())) {
    if (video_session_handler_) {
...
void ap_airplay_connection::post_stop_handler(const request &req, response &res) {
  printf("*** ap_airplay_connection::post_stop_handler\n");
  //DUMP_REQUEST_WITH_CONNECTION(req);
  print_request(req, "");
  ap_casting_media_data_store::get().reset();
...
void ap_airplay_connection::post_action_handler(const request &req, response &res) {
    printf("*** ap_airplay_connection::post_action_handler\n");
  //DUMP_REQUEST_WITH_CONNECTION(req);
    print_request(req, "");
  // req.body (bplist)
  ...
  if (0 == compare_string_no_case(type, "unhandledURLResponse")) {
    ...
    auto fcup_uri_obj = plist_object_dict_get_value(params_dict, "FCUP_Response_URL");
    const char *fcup_uri = nullptr;
    if (0 != plist_object_string_get_value(fcup_uri_obj, &fcup_uri)) {
      res.with_status(bad_request);
      return;
    }

    auto fcup_data_obj = plist_object_dict_get_value(params_dict, "FCUP_Response_Data");
    const uint8_t *fcup_data = nullptr;
    uint64_t data_len = 0;
    if (0 != plist_object_data_get_value(fcup_data_obj, &fcup_data, &data_len)) {
      res.with_status(bad_request);
      return;
    }
    ...
    auto location = ap_casting_media_data_store::get().process_media_data(fcup_uri, fcup_content);
    if (!location.empty()) {
      if (video_session_handler_) {
...
void ap_airplay_connection::get_playback_info_handler(const request &req, response &res) {
  printf("*** ap_airplay_connection::get_playback_info_handler\n");
  // DUMP_REQUEST_WITH_CONNECTION(req);
  print_request(req, "");
  playback_info_t playback_info;
  if (video_session_handler_) {
    video_session_handler_->on_acquire_playback_info(session_id_, playback_info);
  }
  LOGV() << "PlaybackInfo duration: " << playback_info.duration << ", position: " << playback_info.position;
  // Return binary plist
  // clang-format off
...
void ap_airplay_connection::put_setProperty_handler(const request &req, response &res) {
  printf("*** ap_airplay_connection::put_setProperty_handler\n");
  //DUMP_REQUEST_WITH_CONNECTION(req);
  print_request(req, "");
...
void ap_airplay_connection::post_getProperty_handler(const request &req, response &res) {
  printf("*** ap_airplay_connection::post_getProperty_handler\n");
  DUMP_REQUEST_WITH_CONNECTION(req);
  print_request(req, "");
...
std::shared_ptr<xtxp_connection_base> ap_airplay_connection::shared_from_self() { return shared_from_this(); }

void ap_airplay_connection::add_common_header(const request &req, response &res) {
  printf("*** ap_airplay_connection::add_common_header\n");
...
void ap_airplay_connection::validate_user_agent(const request &req) {
  printf("*** ap_airplay_connection::validate_user_agent\n");
  if (!agent_.empty())
    return;
  ...
  if (user_agent_header != req.headers.end()) {
    ...
    ap_casting_content_parser::get_user_agent_version(agent_, agent_version_, user_agent.c_str());
...
#define RH(x) std::bind(&ap_airplay_connection::x, this, std::placeholders::_1, std::placeholders::_2)

void ap_airplay_connection::initialize_request_handlers() {
  printf("*** ap_airplay_connection::initialize_request_handlers\n");
...
void ap_airplay_connection::reverse_connection(const std::string &session) {
  printf("*** ap_airplay_connection::reverse_connection\n");
  apple_session_id_ = session;
  ap_casting_event_connection_manager::get().insert(session, shared_from_self());
  reverse();
}
...
ap_airplay_service::ap_airplay_service(ap_config_ptr &config, uint16_t port /*= 0*/)
  ...
  printf("*** ap_airplay_service::ap_airplay_service\n");

 }

ap_airplay_service::~ap_airplay_service() = default;

void ap_airplay_service::set_handler(ap_handler_ptr &handler) {
  printf("*** ap_airplay_service::set_handler\n");

  handler_ = handler; }

tcp_connection_ptr ap_airplay_service::prepare_new_connection() {
  printf("*** ap_airplay_service::prepare_new_connection\n");
